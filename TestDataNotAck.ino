...
void TCL_RspDataNotAckSentCallback(const TCL_RspDataNotAckSent* response, TCL_Error* error)
{
  ...
  s_indicationLCD_send.hide();
  
  TCL_ErrorSetErrorCode(error, TCL_ERROR_NONE);
}

void TCL_EvtDataNotAckReceivedCallback(const TCL_EvtDataNotAckReceived* event, TCL_Error* error)
{
  ...
  s_indicationLCD_recv.show(millis());
...
void Send_ReqSendDataNotAck(TCL_Error* error)
{
  TCL_Data  data;
